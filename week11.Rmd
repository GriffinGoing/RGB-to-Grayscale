---
title: Converting an RGB Image to Grayscale
...
To convert an RGB image to grayscale, we need only convert the
pixel values to a single flat value, as opposed to the [red, green, blue] format we have when working in RGB images. This is done using the following formula:
```
...
value with this new grayscale value will render a grayscale image as shown below. Note that the current function is not parameterized (the file path is hard-coded), but this could of course easily be done assuming we had the desire to do so.
